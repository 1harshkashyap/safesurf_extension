...
function reportSummaryText(r) {
  if (!r) return 'No report data available.';
  var lines = [];
  lines.push('SafeSurf AI Report Summary');
  lines.push('URL: ' + (r.url || 'Unknown'));
  lines.push('Host: ' + (r.hostname || 'Unknown'));
  lines.push('Score: ' + (r.score || 0) + '/100');
  lines.push('Verdict: ' + (r.label || r.level || 'Unknown'));
...
function exportReportJson(r) {
  ...
  a.href = url;
  a.download = 'safesurf-full-report-' + host + '-' + stamp + '.json';
...
if (btnCopyReport) {
  btnCopyReport.addEventListener('click', function() {
    var text = reportSummaryText(CURRENT_REPORT);
    navigator.clipboard.writeText(text).then(function() {
      btnCopyReport.textContent = '✅ Copied';
    }).catch(function() {
      btnCopyReport.textContent = '❌ Copy failed';
    });
  });
}
...
if (btnExportReport) {
  btnExportReport.addEventListener('click', function() {
    exportReportJson(CURRENT_REPORT);
    btnExportReport.textContent = '✅ Exported';
  });
}
